...
describe "Jat::Plugins::JsonApiLowerCamelCase" do
  ...
  before do
    ...
    serializer_class.id { |object| object }
  end

  it "checks json_api plugin loaded before" do
    serializer_class = Class.new(Jat)
    error = assert_raises(Jat::Error) { serializer_class.plugin :json_api_lower_camel_case }
  ...
  it "returns attributes in lowerCamelCase case" do
    ...
    assert_equal({fooBar: 1}, serializer_class.to_h(true).dig(:data, :attributes))
  end

  it "accepts `fields` in lowerCamelCase format" do
    serializer_class.attribute(:foo_bar, exposed: false) { 1 }

    response = serializer_class.to_h(true, fields: {foo: "fooBar"})
  ...
  it "accepts `include` in lowerCamelCase format" do
    ...
    serializer_class.relationship(:foo_bar, serializer: new_serializer) { 1 }

    response = serializer_class.to_h(true, include: "fooBar")
  ...
  it "returns document meta keys in lowerCamelCase format" do
    serializer_class.document_meta(:user_agent) { "Firefox" }
  ...
  it "returns document jsonapi keys in lowerCamelCase format" do
    serializer_class.jsonapi(:user_agent) { "Firefox" }
  ...
  it "returns document links keys in lowerCamelCase format" do
    serializer_class.document_link(:user_agent) { "Firefox" }
